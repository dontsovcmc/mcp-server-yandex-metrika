...
async def test_ym_counters():
    ...
    with patch("mcp_server_yandex_metrika.server.MetrikaAPI") as M:
        M.return_value.list_counters.return_value = mock
...
async def test_ym_counters_with_search():
    with patch("mcp_server_yandex_metrika.server.MetrikaAPI") as M:
        M.return_value.list_counters.return_value = {"rows": 1, "counters": [{"id": 1, "name": "shop"}]}
        async with create_connected_server_and_client_session(mcp._mcp_server) as s:
...
async def test_ym_counter():
    ...
    with patch("mcp_server_yandex_metrika.server.MetrikaAPI") as M:
        M.return_value.get_counter.return_value = mock
        async with create_connected_server_and_client_session(mcp._mcp_server) as s:
...
async def test_ym_counter_create():
    ...
    with patch("mcp_server_yandex_metrika.server.MetrikaAPI") as M:
        M.return_value.create_counter.return_value = mock
        async with create_connected_server_and_client_session(mcp._mcp_server) as s:
            r = await s.call_tool("ym_counter_create", {"name": "New Site", "site": "new.com"})
            assert not r.isError
            parsed = CounterResponse(**json.loads(r.content[0].text))
...
async def test_ym_counter_update():
    ...
    with patch("mcp_server_yandex_metrika.server.MetrikaAPI") as M:
        M.return_value.update_counter.return_value = mock
        async with create_connected_server_and_client_session(mcp._mcp_server) as s:
            r = await s.call_tool("ym_counter_update", {"counter_id": 12345, "name": "Updated"})
            assert not r.isError
            parsed = CounterResponse(**json.loads(r.content[0].text))
...
async def test_ym_counter_delete():
    with patch("mcp_server_yandex_metrika.server.MetrikaAPI") as M:
        M.return_value.delete_counter.return_value = {"success": True}
        async with create_connected_server_and_client_session(mcp._mcp_server) as s:
            r = await s.call_tool("ym_counter_delete", {"counter_id": 12345})
            assert not r.isError
...
async def test_ym_counter_undelete():
    with patch("mcp_server_yandex_metrika.server.MetrikaAPI") as M:
        M.return_value.undelete_counter.return_value = {"success": True}
        async with create_connected_server_and_client_session(mcp._mcp_server) as s:
            r = await s.call_tool("ym_counter_undelete", {"counter_id": 12345})
            assert not r.isError
            parsed = SuccessResponse(**json.loads(r.content[0].text))
            assert parsed.success is True
