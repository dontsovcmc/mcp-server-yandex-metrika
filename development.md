...
- **Таймауты** — 30 секунд на обычные запросы, 60 секунд на файловые операции. Настраиваемы через env-переменные (`METRIKA_TIMEOUT`, `METRIKA_FILE_TIMEOUT`). В коде использовать `self.timeout` и `self.file_timeout`, не хардкодить числа.
...
- **Хелперы для env-переменных.** Обязательные переменные (токен и т.п.) — через хелперы, которые бросают `RuntimeError` если не задано. Не обращаться к `os.getenv()` напрямую в теле tool-функций.
